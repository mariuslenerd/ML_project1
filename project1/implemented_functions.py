...
def soft_threshold(v, thr):
    ...
    Args:
        ...
        thr (float): threshold
    Returns:
        np.array: soft-thresholded array
    """
...
def compute_logistic_grad_no_reg(y, tx, w):
    ...
    Args:
        ...
        w (np.array): model parameters, shape (D,)
    Returns:
        np.array: gradient of logistic loss, shape (D,)
    """
    N = y.shape[0]
    p = sigmoid(tx @ w)
    weights = make_class_weights(y)
    return (tx.T @ (weights * (p - y))) / N
...
def reg_logistic_lasso(y, tx, lambda_, initial_w, max_iters, gamma, tol=1e-8):
    """
    Perform L1-regularized logistic regression using proximal gradient descent.
    Args:
        y (np.array): true labels, shape (N,)
        tx (np.array): input data, shape (N,D)
        lambda_ (float): regularization strength
